...
/// </summary>
public class PluginStepConfigBuilder<T> : IPluginStepConfigBuilder where T : Entity, new()
{
	...
	private string filteredAttributesCache;
	public string FilteredAttributes => filteredAttributesCache ??= string.Join(",", FilteredAttributesCollection).ToLower();

	public Collection<PluginStepImage> Images { get; } = [];

	public Collection<string> FilteredAttributesCollection { get; } = [];

	internal PluginStepConfigBuilder(string eventOperation, ExecutionStage executionStage)
		: this(EntityLogicalNameCache.GetLogicalName<T>(), eventOperation, executionStage)
	{
	}
